...
# compute reference pressure, refPress, for each layer
...
for {set k [expr $numLayers - 1]} {$k > 0 && $k <= [expr $numLayers - 1]} {incr k -1} {
	...
	puts "vertStress: $vertStress($k)"
	puts "refPress: $refPress($k)"
}
...
# compute soil shear modulus for each layer (kPa)
...
# compute soil elastic modulus for each layer (kPa)
for {set k 0} {$k <= $numLayers} {incr k 1} {
    set E($k)       [expr 2*$G($k)*(1+$nu)]
}

# compute soil bulk modulus for each layer (kPa)
for {set k 0} {$k <= $numLayers} {incr k 1} {
...
if {$earthquake == "13June2011a" && $comp == "000"} {
    print output/PPnodeInfo.txt -node
} 
...
if {$earthquake == "13June2011a" && $comp == "000"} {
    print output/nodeInfo.txt -node
} 
...
set g -9.81
set bulkWater 2.2e6
set materialProperties [open materialProperties.dat w]
...
set motionSteps [expr int($total_time/0.005)]
...
#set omega1  [expr 2*$pi*1.69]
# upper frequency
#set omega2  [expr 2*$pi*20]
#set omega2  [expr 2*$pi*8.46]
...
# # record nodal displacment, acceleration, and porepressure
# eval "recorder Node -file output/$GMversion/$gMotionName.disp.out -time -dT $dT -nodeRange 1 $nNodeT -dof 1 2  disp"
# eval "recorder Node -file output/$GMversion/$gMotionName.vel.out -time -dT $dT -nodeRange 1 $nNodeT -dof 1 2  vel"
# eval "recorder Node -file output/$GMversion/$gMotionName.acc.out -time -dT $dT -nodeRange 1 $nNodeT -dof 1 2  accel"
# eval "recorder Node -file output/$GMversion/$gMotionName.porePressure.out -time -dT $dT -node $nodeList3 -dof 3 vel"
# record horizontal velocity at the top node
eval "recorder Node -file output/$GMversion/$gMotionName.vel.out -time -dT $dT -node $nNodeT -dof 1 vel"
# record horizontal acceleration at the top node
eval "recorder Node -file output/$GMversion/$gMotionName.acc.out -time -dT $dT -node $nNodeT -dof 1 accel"
# # record elemental stress and strain
# recorder Element -file output/$GMversion/$gMotionName.stress1.out   -time -dT $dT  -eleRange 1 $nElemT  material 1 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress2.out   -time -dT $dT  -eleRange 1 $nElemT  material 2 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress3.out   -time -dT $dT  -eleRange 1 $nElemT  material 3 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress4.out   -time -dT $dT  -eleRange 1 $nElemT  material 4 stress 3 
# recorder Element -file output/$GMversion/$gMotionName.stress5.out   -time -dT $dT  -eleRange 1 $nElemT  material 5 stress 3 
# recorder Element -file output/$GMversion/$gMotionName.stress6.out   -time -dT $dT  -eleRange 1 $nElemT  material 6 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress7.out   -time -dT $dT  -eleRange 1 $nElemT  material 7 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress8.out   -time -dT $dT  -eleRange 1 $nElemT  material 8 stress 3
# recorder Element -file output/$GMversion/$gMotionName.stress9.out   -time -dT $dT  -eleRange 1 $nElemT  material 9 stress 3
# recorder Element -file output/$GMversion/$gMotionName.strain1.out   -time -dT $dT  -eleRange 1 $nElemT  material 1 strain
# recorder Element -file output/$GMversion/$gMotionName.strain2.out   -time -dT $dT  -eleRange 1 $nElemT  material 2 strain
# recorder Element -file output/$GMversion/$gMotionName.strain3.out   -time -dT $dT  -eleRange 1 $nElemT  material 3 strain
# recorder Element -file output/$GMversion/$gMotionName.strain4.out   -time -dT $dT  -eleRange 1 $nElemT  material 4 strain
# recorder Element -file output/$GMversion/$gMotionName.strain5.out   -time -dT $dT  -eleRange 1 $nElemT  material 5 strain
# recorder Element -file output/$GMversion/$gMotionName.strain6.out   -time -dT $dT  -eleRange 1 $nElemT  material 6 strain
# recorder Element -file output/$GMversion/$gMotionName.strain7.out   -time -dT $dT  -eleRange 1 $nElemT  material 7 strain
# recorder Element -file output/$GMversion/$gMotionName.strain8.out   -time -dT $dT  -eleRange 1 $nElemT  material 8 strain
# recorder Element -file output/$GMversion/$gMotionName.strain9.out   -time -dT $dT  -eleRange 1 $nElemT  material 9 strain
